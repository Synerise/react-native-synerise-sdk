...
var SyneriseInitializer = /** @class */ (function () {
    ...
    SyneriseInitializer.prototype.withApiKey = function (apiKey) {
        RNSynerise.withApiKey(apiKey);
        return this;
    };
    ...
    SyneriseInitializer.prototype.withInitialDoNotTrack = function (initialDoNotTrack) {
        RNSynerise.withInitialDoNotTrack(initialDoNotTrack);
        return this;
    };
...
var Synerise = /** @class */ (function () {
    ...
    Synerise.changeApiKey = function (apiKey, config) {
        if (SyneriseInitializer.isSyneriseInitialized()) {
            var configObject = config === null || config === void 0 ? void 0 : config.toObject();
            SyneriseModuleConnector_1.SyneriseModuleConnector.invokeMethod(RNSynerise.changeApiKey, [apiKey, configObject]);
            // A new API key means a new workspace, so context from the previous one must not be evaluated
            // against its campaigns. The setter clears the JS mirror and pushes the empty context natively,
            // which keeps both platforms consistent regardless of what the native SDK resets on its own.
            Synerise.Injector.inAppContext = {};
        }
        else {
